...
#elif defined(_WIN32)
extern "C" {
	uint64_t __fastcall _kAFL_hypercall(uint64_t backdoor, uint64_t action, uint64_t arg1);
}
static inline uint64_t kAFL_hypercall(uint64_t action, uint64_t arg1) {
	return _kAFL_hypercall(HYPERCALL_KAFL_RAX_ID, action, arg1);
}
#endif
...
static void hprintf(const char* format, ...) {
	static char __declspec(align(4096)) hprintf_buffer[HPRINTF_MAX_SIZE];
